...
/**
 * @fileOverview A flow that generates creative descriptions for Pastelería Hijitos' craftsmanship values.
 *
...
const GenerateCraftsmanshipDescriptionInputSchema = z.object({
  currentDescription: z
    .string()
    .describe('La descripción actual de los valores de artesanía de Pastelería Hijitos.'),
});
...
const GenerateCraftsmanshipDescriptionOutputSchema = z.object({
  description: z
    .string()
    .describe('La descripción creativa generada por IA de los valores de artesanía de Pastelería Hijitos.'),
});
...
const prompt = ai.definePrompt({
  ...
  prompt: `Eres un redactor creativo para Pastelería Hijitos, una panadería artesanal conocida por su compromiso con la artesanía, su talentoso personal y su enfoque en la comunidad. La descripción actual es: {{{currentDescription}}}. Genera una nueva y atractiva descripción que capture la esencia de los valores de Pastelería Hijitos. Enfócate en el arte, el talento y la comunidad. Sé conciso, pero detallado. Sé lo más evocador y persuasivo posible. La descripción generada debe tener solo unas pocas frases. La respuesta debe estar en español.`,
});
